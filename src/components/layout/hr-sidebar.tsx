...
import { HR_SIDENAV_ITEMS } from '@/lib/hr-constants';
...
export function HrSidebar({ isSidebarOpen, setSidebarOpen, isCollapsed, setIsCollapsed }: { 
      ...
      <div className="flex items-center">
        <Avatar className="h-10 w-10 bg-primary text-primary-foreground font-bold">
          <AvatarFallback className='bg-orange-500'>
            {userName ? userName.charAt(0).toUpperCase() : 'HR'}
          </AvatarFallback>
        </Avatar>
        {!isCollapsed && <h1 className="ml-3 text-2xl font-bold text-white">{getFirstName(userName) || 'HR'}</h1>}
      </div>
      {!isCollapsed && (
            ...
            <DropdownMenuTrigger asChild>
              <div className="flex items-center cursor-pointer group w-full">
                <Avatar className="h-10 w-10 font-bold">
                  <AvatarFallback className='bg-orange-500 text-primary-foreground'>{userName ? userName.charAt(0).toUpperCase() : 'H'}</AvatarFallback>
                </Avatar>
                {!isCollapsed && (
                  <div className="ml-3">
                    <p className="text-sm font-medium">{userName || 'HR User'}</p>
                    <p className="text-xs text-muted-foreground group-hover:text-sidebar-foreground/80">
                      {userEmail || 'hr@nexus.com'}
                    </p>
                  </div>
                )}
              </div>
            ...
            <DropdownMenuContent className="w-56 mb-2 ml-2 bg-popover border-border text-popover-foreground" align="end" forceMount>
                 <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                        <p className="text-sm font-medium leading-none">{userName || 'HR User'}</p>
                ...
                <DropdownMenuSeparator/>
                <DropdownMenuItem>
                    <Link href="/hr-dashboard/profile" className="flex items-center w-full">
                        <span>Profile</span>
                    </Link>
                </DropdownMenuItem>
    ...
    <>
      ...
      <aside className={cn(
          ...
          <div className='flex flex-col h-full'>
            <SidebarStructure isCollapsed={isCollapsed} setIsCollapsed={setIsCollapsed} setSidebarOpen={setSidebarOpen}/>
