...
import { BarChart3, CalendarDays, CheckCircle2, Clock3, FileCheck2, LayoutDashboard, Search, Settings, XCircle, AlertTriangle, Download, FileText, MapPin, User } from "lucide-react";
...
export function HrDashboardShell() {
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="xl:col-span-2">
          <CardHeader>
            <CardTitle  className="text-md md:text-base">Weekly Attendance Trend</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle  className="text-md md:text-base">Today's Distribution</CardTitle>
          </CardHeader>
          <CardContent className="h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-md md:text-base">Employee Attendance</CardTitle>
          <span className="text-xs text-muted-foreground">{filteredEmployees.length} records</span>
        </CardHeader>
        <CardContent className="overflow-x-auto pb-4  w-[90vw] md:w-full">
          <Table>
            <TableHeader>
              <TableRow>
  ...
  const renderLeave = () => (
    <Card>
      <CardHeader>
        <CardTitle  className="text-md md:text-base">Pending Leave Requests</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {hrLeaveRequests.map((item) => (
          <div key={item.id} className="flex flex-wrap  items-center justify-between rounded-lg border p-3">
            <div>
              <p className="text-sm md:font-medium">{item.name}</p>
  ...
  const renderCorrections = () => (
    <Card>
      <CardHeader>
        <CardTitle  className="text-md md:text-base">Attendance Corrections</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
  ...
  const renderReports = () => (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        <CardHeader><CardTitle  className="text-md md:text-base">Alerts Panel</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          {hrAlerts.map((alert) => (
      ...
      <Card>
        <CardHeader><CardTitle  className="text-md md:text-base">Compliance Snapshot</CardTitle></CardHeader>
  ...
  const renderSettings = () => (
    <Card>
      <CardHeader><CardTitle  className="text-md md:text-base">HR Settings</CardTitle></CardHeader>
      <CardContent className="space-y-3 text-sm text-muted-foreground">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between">
        <div>
          <h1 className="text-xl md:text-2xl font-bold">HR Attendance Dashboard</h1>
          <p className="text-sm text-muted-foreground">Role-based HR view with navigation and mock data.</p>
        </div>
